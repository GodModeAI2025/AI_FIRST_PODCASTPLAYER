...
public final class AppModel {
    ...
    /// So viele Stellen mit Zeitmarke lesen „Für dich“, die Vorschläge und
    /// der Chat. Die Voreinstellung des Speichers liefert
    ...
    /// YouTube-Videos und einzelne Beiträge zählen mit, wenn Supadata sie
    /// holen darf.
    private func newestCandidates(in list: [Episode]) -> ArraySlice<Episode> {
        ...
        if source?.kind == .youTubeChannel {
            return list.filter { allowsAutomaticCaptions($0) }.prefix(episodesPerSource)
        }
        var picked = PreparationCandidates.newest(in: list, of: source, perSource: episodesPerSource)
        if source?.isSubscribed == false {
            let taken = Set(picked.map(\.id))
            picked += list.filter { !taken.contains($0.id) && allowsAutomaticCaptions($0) }
        }
        return ArraySlice(picked)
    }
