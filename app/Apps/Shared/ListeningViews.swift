...
struct EpisodeDetailView: View {
    ...
    var body: some View {
        List {
            ...
            if let stage {
                ...
                Section("Erschliessung") { Label(detail, systemImage: "clock") }
            }
                        ...
                        Button {
                            ...
                            HStack(alignment: .firstTextBaseline) {
                                ...
                                Text(chapter.title)
                                    ...
                                    .fontWeight(isCurrent && player.currentChapter?.start == chapter.start ? .semibold : .regular)
                                Spacer()
                                if hasHeard(from: chapter) {
            ...
            if !passages.isEmpty {
                Section {
                    ForEach(passages) { passage in
                        if let range = passage.range {
                            Button {
                                model.playEpisode(episode, at: range.start.seconds)
                            } label: {
                                PassageRow(passage: passage, range: range,
                                           heard: model.hasHeard(range, in: passage.mediaVersionID))
                            }
                        }
                    }
                } header: {
                    Text("Erschlossene Stellen")
                } footer: {
                    Text("Antippen springt an die Stelle. Ein Haken heisst: schon gehört.")
                }
            }
        ...
        #endif
        .task(id: model.stages[episode.id]) { passages = await model.evidence(forEpisode: episode.id) }
    ...
    private func hasHeard(from chapter: Chapter) -> Bool {
        ...
        return model.hasHeard(
            MediaTimeRange(start: chapter.start, end: MediaTime(milliseconds: endMs)), in: id)
    }
}
